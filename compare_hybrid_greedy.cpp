...
struct MCMF {
    ...
    void ae(int from, int to, ll cap, db cost) {
        if (from == to) return;
    ...
    void path(int s) {
        ...
        FOR (i, N) pi[i] = min(pi[i] + dist[i], 1e18);
    }

    pair<ll, db> maxflow(int s, int t) {
        ll totflow = 0;
        db totcost = 0;
        while (path(s), seen[t]) {
            ll fl = INF;
...
// Hungarian (minimization) for doubles; 1-indexed matrix expected.
static pair<db, vi> hungarian_min(const vector<vector<db>>& a) {
    int n = (int)a.size() - 1;
    vi p(n + 1), way(n + 1);
    vector<db> u(n + 1), v(n + 1);
    for (int i = 1; i <= n; ++i) {
        p[0] = i;
        vector<db> minv(n + 1, 1e100);
    ...
    db value = -v[0];
    return {value, assign}; // minimal cost and assignment (row i -> column assign[i])
}

// Repeated Hungarian: maximize sum over k rounds; forbid reusing the same (i,j) pair.
static db repeated_hungarian_max(const vt<vt<db>>& w, int k) {
    int n = (int)w.size();
    ...
    db latest = 0;
    for (int r = 0; r < k; ++r) {
        vector<vector<db>> cost(n + 1, vector<db>(n + 1, 0));
        for (int i = 1; i <= n; ++i)
            for (int j = 1; j <= n; ++j)
                cost[i][j] = used[i - 1][j - 1] ? BIG : -w[i - 1][j - 1];
        auto [min_cost, assign] = hungarian_min(cost);
        latest = -min_cost;
        total += -min_cost; // negate because we minimized -w
        for (int i = 1; i <= n; ++i) used[i - 1][assign[i] - 1] = true;
    }
    return latest;
}

// Hybrid method: use MCMF to choose first (k-1) disjoint matchings (maximizing sum
// of weights), then run Hungarian once on the remaining edges to find the k-th.
static db hybrid_mcmf_then_hungarian(const vt<vt<db>>& w, int k) {
    int n = (int)w.size();
    // build weight matrix and track how many times each edge is already used
    vector<vector<int>> used(n, vector<int>(n, 0));

    db total = 0;

    // First pick k-1 disjoint matchings via MCMF (equivalent to earlier setting)
    if (k > 1) {
        // Build cost matrix for MCMF: negative weights (for max-sum)
        vt<vt<db>> mat(n, vt<db>(n));
        FOR (i, n) FOR (j, n) mat[i][j] = -w[i][j];
        ...
        // flow should be (k-1)*n, but if k==1 we skipped this block.
        assert(flow == n * (k - 1));
        total += -cost;

        // Derive which edges were used: any edge with positive flow from i to j+n
        FOR (i, n) {
            for (auto &e : mcmf.ed[i]) if (e.to >= 0 && e.to < 2*n && e.to >= n && e.flow > 0) {
                int j = e.to - n;
                used[i][j] = 1;
            }
        }
    ...
    // Now build cost matrix for the final Hungarian, forbidding already-used edges.
    const db BIG = 1e9;
    vector<vector<db>> cost(n + 1, vector<db>(n + 1, 0));
    for (int i = 1; i <= n; ++i)
        for (int j = 1; j <= n; ++j)
...
signed main() {
    ...
    const int n = 10;
    // Exponential distribution with rate lambda = 1 for edge weights
    std::exponential_distribution<db> exp_dist(1.0);
    auto next_double = [&] () {
        return exp_dist(mt);
    };
    const int reps = 10000;
    FOR (k, 1, n+1) {
        db tot_greedy = 0, tot_hybrid = 0;
        FOR (reps) {
            vt<vt<db>> w(n, vt<db>(n));
            FOR (i, n) FOR (j, n) w[i][j] = next_double();

            // Strategy 1: k disjoint matchings via repeated Hungarian.
            db greedy_val = repeated_hungarian_max(w, k);

            // Strategy 2: first k-1 matchings via MCMF, then 1 Hungarian.
            db hybrid_val = hybrid_mcmf_then_hungarian(w, k);

            tot_greedy += greedy_val;
            tot_hybrid += hybrid_val;
        }
        println(tot_greedy / reps, tot_hybrid / reps);
    }
}